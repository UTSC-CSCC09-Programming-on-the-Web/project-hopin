...
async function testUnauthorizedAccess() {
  ...
  for (const endpoint of protectedEndpoints) {
    ...
    log(`${endpoint.method} ${endpoint.path}: ${response.status} ${isBlocked ? '✅' : '❌'}`, 
      isBlocked ? 'green' : 'red');
...
async function testUserCreation() {
  ...
  log(`User Signup: ${signupResponse.status} ${signupSuccess ? '✅' : '❌'}`, 
  ...
  log(`User Login: ${loginResponse.status} ${loginSuccess ? '✅' : '❌'}`, 
...
async function testSessionFixation() {
  ...
  log(`Tokens are different: ${differentTokens ? '✅' : '❌'}`, 
...
async function runQuickTests() {
  ...
  try {
    ...
    log(`Unauthorized Access Protection: ${results.unauthorizedAccess ? 'PASS ✅' : 'FAIL ❌'}`, 
      results.unauthorizedAccess ? 'green' : 'red');
    log(`User Creation/Login: ${results.userCreation ? 'PASS ✅' : 'FAIL ❌'}`, 
      results.userCreation ? 'green' : 'red');
    log(`Session Fixation Protection: ${results.sessionFixation ? 'PASS ✅' : 'FAIL ❌'}`, 
