...
async function runAllSecurityTests() {
  ...
  const tests = [
    {
      ...
      description:
        "Authentication, subscription, rate limiting, and input validation",
    },
    {
  ...
  log(
    ...
    successfulTests === results.length ? "green" : "yellow",
  );
  ...
  if (successfulTests === results.length) {
    ...
    log(
      "Your application security is robust and ready for production.",
      "green",
    );
  } else if (successRate >= 80) {
    log("\n⚠️  MOST SECURITY TESTS PASSED", "yellow");
    log(
      "Some issues detected. Review failed tests and address before production.",
      "yellow",
    );
  } else {
    log("\n🚨 CRITICAL SECURITY ISSUES DETECTED", "red");
    log(
      "Multiple test failures indicate serious security vulnerabilities.",
      "red",
    );
  ...
  if (failedTests > 0) {
    ...
    log(
      "4. Review and update security tests as the application evolves",
      "blue",
    );
  }
