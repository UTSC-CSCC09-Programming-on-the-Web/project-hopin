...
async function testRateLimitingComprehensive() {
  ...
  const successfulRequests = loginResults.filter(
    (r) => r.status === 401,
  ).length;
  const rateLimitedRequests = loginResults.filter(
    (r) => r.status === 429,
  ).length;
  ...
  log(
    ...
    rateLimitedRequests > 10 ? "green" : "red",
  );
  log(`  ├─ Errors: ${errorRequests}`, "blue");
  log(
    `  └─ Result: ${rateLimitedRequests > 10 ? "PASS" : "FAIL"}`,
    rateLimitedRequests > 10 ? "green" : "red",
  );
  ...
  const successfulSignups = signupResults.filter(
    (r) => r.status === 201,
  ).length;
  const rateLimitedSignups = signupResults.filter(
    (r) => r.status === 429,
  ).length;
  ...
  log(
    `  ├─ Rate limited (429): ${rateLimitedSignups}`,
    rateLimitedSignups > 5 ? "green" : "yellow",
  );
  log(`  ├─ Rejected (400): ${rejectedSignups}`, "blue");
  log(
    `  └─ Result: ${rateLimitedSignups > 0 || successfulSignups < 10 ? "PASS" : "FAIL"}`,
    rateLimitedSignups > 0 ? "green" : "yellow",
  );
  ...
  if (user) {
    ...
    const subscriptionBlocked = apiResults.filter(
      (r) => r.status === 402,
    ).length;
    ...
    log(
      `  ├─ Rate limited (429): ${rateLimitedAPIs}`,
      rateLimitedAPIs > 0 ? "green" : "yellow",
    );
    log(
      `  ├─ Subscription blocked (402): ${subscriptionBlocked}`,
      subscriptionBlocked > 0 ? "green" : "blue",
    );
    ...
    const protectionWorks =
      rateLimitedAPIs > 0 || subscriptionBlocked === apiResults.length;
    log(
      `  └─ Result: ${protectionWorks ? "PASS" : "FAIL"}`,
      protectionWorks ? "green" : "red",
    );
  } else {
  ...
  const totalBlocked = mixedResults.filter(
    (r) => r.status === 401 || r.status === 403,
  ).length;
  const totalSuccessful = mixedResults.filter(
  ...
  log(
    ...
    totalRateLimited > 5 ? "green" : "yellow",
  );
  log(
    `  ├─ Blocked (401/403): ${totalBlocked}`,
    totalBlocked > 0 ? "green" : "blue",
  );
  log(
    `  ├─ Successful: ${totalSuccessful}`,
    totalSuccessful < 15 ? "green" : "yellow",
  );
  log(
    `  └─ Result: ${totalRateLimited > 5 ? "PASS" : "PARTIAL"}`,
    totalRateLimited > 5 ? "green" : "yellow",
  );
  ...
  for (let batch = 0; batch < 3; batch++) {
    ...
    const batchRateLimited = batchResults.filter(
      (r) => r.status === 429,
    ).length;
    log(
      `    └─ Batch ${batch + 1}: ${batchRateLimited}/10 rate limited`,
      batchRateLimited > 0 ? "green" : "blue",
    );
  ...
  const totalProgressiveRateLimited = progressiveResults.filter(
    (r) => r.status === 429,
  ).length;
  ...
  log(
    `  └─ Result: ${progressiveWorks ? "PASS" : "PARTIAL"}`,
    progressiveWorks ? "green" : "yellow",
  );
  ...
  if (passedTests === totalTests) {
    ...
    log(
      "The system effectively prevents abuse through rate limiting.",
      "green",
    );
