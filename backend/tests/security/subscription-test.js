...
async function createAndLoginUser() {
  ...
  if (signupResponse.status !== 201) {
    log(
      `❌ Failed to create user: ${JSON.stringify(signupResponse.data)}`,
      "red",
    );
    return null;
  }
...
async function testSubscriptionEnforcement(token) {
  ...
  for (const endpoint of protectedEndpoints) {
    ...
    log(
      `${endpoint.name}: ${response.status} ${isBlocked ? "✅" : "❌"}`,
      isBlocked ? "green" : "red",
    );
    ...
    if (!isBlocked) {
      allBlocked = false;
      log(
        `  └─ Expected 402, got ${response.status}: ${JSON.stringify(response.data)}`,
        "red",
      );
    }
  }
...
async function testRateLimiting() {
  ...
  log(
    `Rate Limited: ${rateLimited}/${total}`,
    rateLimited > 0 ? "green" : "yellow",
  );
  log(
    `Subscription Blocked: ${blocked}/${total}`,
    blocked > 0 ? "green" : "red",
  );
...
async function runSubscriptionTests() {
  ...
  try {
    ...
    if (userResult) {
      ...
      results.subscriptionEnforcement = await testSubscriptionEnforcement(
        userResult.token,
      );
    ...
    log(
      `Subscription Enforcement: ${results.subscriptionEnforcement ? "PASS ✅" : "FAIL ❌"}`,
      results.subscriptionEnforcement ? "green" : "red",
    );
    log(
      `Rate Limiting: ${results.rateLimiting ? "PASS ✅" : "FAIL ❌"}`,
