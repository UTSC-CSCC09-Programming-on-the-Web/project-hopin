...
async function testAuthenticationSecurity() {
  ...
  logTest(
    ...
    allBlocked
      ...
      : "Some endpoints allow unauthorized access",
  );
  ...
  if (user) {
    ...
    logTest(
      ...
      differentTokens
        ...
        : "Same token reused across logins",
    );
    ...
    logTest(
      ...
      "FAIL",
      "critical",
      "Could not test - user creation failed",
    );
  }
  ...
  logTest(
    ...
    inputValidationWorks ? "PASS" : "FAIL",
    "critical",
    inputValidationWorks
      ? "Authentication rejects malicious inputs safely"
...
async function testSubscriptionEnforcement() {
  ...
  const subscriptionEndpoints = [
    ...
    {
      path: "/api/users/by-email?email=test@example.com",
      method: "GET",
  ...
  logTest(
    ...
    "critical",
    allBlocked
      ? "All protected endpoints require active subscription (402 errors)"
      : "Some endpoints allow access without subscription",
  );

  // Test subscription status endpoint (should work without subscription)
  const statusResponse = await makeRequest(
    `${BASE_URL}/api/users/${user.userId}/subscription-status`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
  ...
  const statusWorks =
    statusResponse.status === 200 && statusResponse.data?.subscriptionStatus;
  logTest(
    "Subscription Status Check",
    statusWorks ? "PASS" : "FAIL",
    "normal",
    statusWorks
      ? `Status: ${statusResponse.data?.subscriptionStatus}`
...
async function testRateLimiting() {
  ...
  logTest(
    ...
    rateLimitedSignups > 0 ? "PASS" : "FAIL",
    "normal",
...
async function testInputValidation() {
  ...
  logTest(
    "XSS Prevention",
    xssProtected ? "PASS" : "FAIL",
    "critical",
    xssProtected
      ? "Malicious scripts properly handled"
...
async function runSecurityTests() {
  ...
  try {
    ...
    log(
      ...
      testResults.passed > 0 ? "green" : "red",
    );
    log(
    ...
    log(
      `Critical Failures: ${testResults.critical}`,
      testResults.critical === 0 ? "green" : "red",
    );
    log(
