...
class RateLimitTester {
  ...
  async testSignupRateLimit() {
    ...
    try {
      ...
      for (let i = 1; i <= 8; i++) {
        try {
          ...
          if (response.status === 200 || response.status === 201) {
            successCount++;
            this.log(`  Attempt ${i}: Success (${response.status})`, colors.green);
          }
        } catch (error) {
          if (error.response?.status === 429) {
            ...
            this.log(`  Attempt ${i}: Other error (${error.response?.status || 'network'})`, colors.red);
          }
        }
      ...
      this.logResult('Enhanced Signup Rate Limiting', false, `Test failed: ${error.message}`);
      return false;
    }
  }
  ...
  async testSigninRateLimit() {
    ...
    try {
      ...
      for (let i = 1; i <= 10; i++) {
        try {
          ...
          if (error.response?.status === 429) {
            ...
            this.log(`  Attempt ${i}: Auth failed (401) - expected`, colors.blue);
          }
        }
      ...
      this.logResult('Enhanced Signin Rate Limiting', false, `Test failed: ${error.message}`);
      return false;
    }
  }
  ...
  async testProgressiveBlocking() {
    ...
    try {
        ...
        for (let i = 1; i <= 4; i++) {
          try {
            ...
            if (error.response?.status === 429) {
              ...
              this.log(`    Rate limited - Retry after: ${retryAfter}s`, colors.yellow);
              
              if (retryAfter > lastRetryAfter && lastRetryAfter > 0 && retryAfter < 3600) {
                progressiveDelayDetected = true;
                this.log(`    ✓ Progressive delay detected! (${lastRetryAfter}s → ${retryAfter}s)`, colors.green);
              }
      ...
      this.logResult('Progressive Blocking', false, `Test failed: ${error.message}`);
      return false;
    }
  }
  ...
  async testSubscriptionRateLimit() {
    this.log('\n💳 Testing Subscription Endpoint Rate Limiting...', colors.cyan);
    ...
    try {
      ...
      for (let i = 1; i <= 5; i++) {
        try {
          ...
          if (error.response?.status === 429) {
            blockedCount++;
            this.log(`  Checkout attempt ${i}: Rate limited (429)`, colors.yellow);
          } else if (error.response?.status === 401) {
            this.log(`  Checkout attempt ${i}: Unauthorized (401) - expected`, colors.blue);
          }
        }
      ...
      this.logResult('Subscription Rate Limiting', false, `Test failed: ${error.message}`);
      return false;
    }
  }
  ...
  async testUserEndpointRateLimit() {
    ...
    try {
      ...
      for (let i = 1; i <= 25; i++) {
        try {
          await axios.get(`${BASE_URL}/users/by-email?email=test${i}@example.com`);
          attemptCount++;
        } catch (error) {
          attemptCount++;
          if (error.response?.status === 429) {
            ...
            if (i <= 5) this.log(`  User request ${i}: Unauthorized (401) - expected`, colors.blue);
          }
        }
      ...
      this.logResult('User Endpoint Rate Limiting', false, `Test failed: ${error.message}`);
      return false;
    }
  }
  ...
  async runAllTests() {
    ...
    this.log(`📈 Success Rate: ${successRate}%`, successRate >= 80 ? colors.green : colors.yellow);
