...
export function validateRequestPayload (schemas) {
  return (req, res, next) => {
    ...
    const validatePart = (part, schema, partName) => {
      ...
      const unexpectedKeys = partKeys.filter((key) => !schemaKeys.includes(key));
      unexpectedKeys.forEach((key) => {
        errors.push(`Unexpected ${partName} parameter: ${key}`);
      });

      // Validate and coerce expected parameters
      for (const [key, { required, type, coerce = false }] of Object.entries(schema)) {
        let value = part[key];
          ...
          if (coerce && typeof value !== type) {
            if (type === "number" && typeof value === "string") {
              ...
              if (!isNaN(numValue)) {
                ...
                errors.push(`Invalid ${partName} parameter ${key}: cannot coerce "${value}" to ${type}`);
              }
              ...
              if (typeof value === "string") {
                ...
                else errors.push(`Invalid body parameter ${key}: Cannot coerce "${value}" to boolean`);
              } else if (typeof value === "number") {
                req.body[key] = Boolean(value);
              } else if (typeof value !== "boolean") {
                errors.push(`Invalid body parameter ${key}: Cannot coerce "${value}" to boolean`);
              }
            } else {
              errors.push(`Cannot coerce ${partName} parameter ${key} to ${type}`);
            }
          } else if (typeof value !== type) {
            errors.push(`Invalid type for ${partName} parameter ${key}: expected ${type}, got ${typeof value}`);
          }
        }
      }
    };
