    ...
    switch (event.type) {
      case "checkout.session.completed":
        ...
        try {
          ...
          console.log(
            "Error updating user's subscription data in the database:",
            error,
          );
      ...
      case "customer.subscription.updated":
        ...
        if (!user) {
          console.error(
            "User not found with corresponding stripe customer ID: ",
            customerId,
          );
      ...
      case "customer.subscription.deleted":
        try {
          ...
          console.log(
            "Error updating subscription deletion in database:",
            error,
          );
  ...
  async (req, res) => {
    try {
      ...
      if (!userSubscription) {
        ...
        return res
          .status(404)
          .json({ error: "User does not have a subscription" });
      }
