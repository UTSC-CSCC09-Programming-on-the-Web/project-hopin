...
function SubscriptionDetail() {
  ...
  useEffect(()=>{
    ...
    if (currentUser?.subscriptionStatus === "active") {
      setLoading(true);
      paymentApi.getSubscriptionDetail(currentUser.id)
      .then(res => {
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">            
            ...
            <div className="px-6 py-6">
              ...
              <div className="flex items-baseline gap-2 mb-2 mt-4 ml-1">
                <span className="text-2xl font-bold text-gray-900"> {subscriptionData.name[0].toUpperCase() + subscriptionData.name.slice(1)} Plan</span>
                <span className="text-lg text-gray-500 font-medium">{subscriptionData.interval[0].toUpperCase() + subscriptionData.interval.slice(1)}</span>
              </div>
              <div className="flex items-center text-gray-600 mb-2 ml-1">
                <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <span className="text-sm">Next payment of ${subscriptionData.amount} is on {new Date(subscriptionData.nextBillingDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric"})}</span>
              </div>
            </div>
      ...
      <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">You don't have an active subscription yet. Subscribe to unlock HopIn.</p>
          </div>
            ...
            <div className="px-6 py-6">
              <ul className="space-y-3">
                <li className="flex items-start">
                  ...
                  <span className="text-gray-700">Create and join your friends to share rides</span>
                </li>
                <li className="flex items-start">
                  <svg className="w-5 h-5 text-green-500 mt-0.5 mr-3 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                  </svg>
                  <span className="text-gray-700">See friends on map in real-time</span>
                </li>
                <li className="flex items-start">
                  <svg className="w-5 h-5 text-green-500 mt-0.5 mr-3 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                  </svg>
                  <span className="text-gray-700">Automatic calculation of costs</span>
                </li>
              </ul>
            </div>
