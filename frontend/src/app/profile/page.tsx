...
function UserProfile() {
  ...
  const handleProfileUpdate = (userId: number, formElement: HTMLFormElement) => {
    setLoading(true);
    ...
    if (formData.avatar instanceof File) {
      console.log('Using selected avatar file:', formData.avatar.name);
      // Ensure the File object is in the FormData
      // Remove any existing avatar entry that might have come from the form
      data.delete('avatar');
      data.append('avatar', formData.avatar);
    } else if (typeof formData.avatar === 'string' && formData.avatar) {
      console.log('Using existing avatar URL:', formData.avatar);
      // The hidden input should handle this, but let's make sure
      if (!data.get('avatar')) {
        data.append('avatar', formData.avatar);
      }
    }
    ...
    userApi.updateUser(userId, data)
      .then((updatedUser) => {
        ...
        setUser(updatedUser);
        // Reset form data to the updated user data
        setFormData({
          ...
          {isEditing ? (
            ...
            >
              {imagePreview ? (
                ...
                user && user.avatar ? (
                  ...
                  <span className="text-2xl font-bold opacity-70">{user && user.username ? user.username.charAt(0).toUpperCase() : ''}</span>
                )
              )}
            ...
            user && user.avatar ? (
              <>
                ...
                />
                <span id="avatar-fallback" className="text-2xl font-bold hidden">{user.username.charAt(0).toUpperCase()}</span>
              </>
            ) : (
        ...
        {isEditing ? (
            <div className="w-full md:w-3/4">
              ...
              <form onSubmit={(e) => {
                e.preventDefault();
                if (user) {
                  handleProfileUpdate(user.id, e.currentTarget);
                }
              }}>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => setFormData({ ...formData, homeAddr: e.target.value })} 
                      className="w-full p-2 border rounded"
                    />
                  </div>
                  ...
                  {typeof formData.avatar === 'string' && formData.avatar && !imagePreview && (
                    <input 
                      type="hidden" 
                      name="avatar" 
